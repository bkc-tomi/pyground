...
|----|----|----|----|----|----|----|----|
|0|トップ|トップページ| /top/ | top | /top/index.html |サイトの説明、お知らせ||
|0|ユーザー|登録ページ| /user/register/ | register | /user/register.html |登録内容の入力、フォーム送信|登録メールの送信メッセージを表示|
...
|0|ユーザー|ログインページ| /user/login/ | login | /user/login.html |ログイン情報の入力、フォーム送信||
...
|0|ユーザー|退会ページ| /user/<int: user_id>/withdrawal/ | withdrawal | /user/withdrawal.html |退会についての説明、大会の確認||
|0|ユーザー|退会処理| /user/<int: user_id>/run/withdrawal/ | run_withdrawal ||退会処理|退会ページからPOSTで取得|
|0|ユーザー|プロフィール詳細| /user/<int: user_id>/detail | detail | /user/detail.html |ユーザー情報の表示、フォローリスト・フォロワーリストの一部表示||
|0|ユーザー|プロフィール編集ページ| /user/<int: user_id>/edit/ | edit | /user/edit.html |ユーザー情報の編集、フォーム送信||
...
||フレンド|フォロー一覧ページ| /friend/<int: user_id>/follow/ | follow | /follow/follow.html |フォローしているユーザーを表示、フォロー解除||
...
||問題|問題一覧ページ| /question/list/ | questions | /question/index.html |問題一覧の表示(全て)、ブックマークフォーム送信||
...
||問題|問題編集・新規作成ページ| /question/<int: question_id>/edit/ | edit | /question/edit.html |問題の作成・既存の問題の編集内容を入力、フォーム送信||
...
||プレイグランド|入力ページ| /playground/<int: code_id> | index | /playground/index.html |コード入力、実行フォーム送信、保存フォーム送信||
||プレイグランド|実行処理| /playground/<int: code_id>/run/ | run ||コードの実行、問題との答え合わせ、正解者登録|入力ページからPOSTで取得|
||プレイグランド|問題ページ| /playground/<int: question_id>/<int: code_id> | question | /playground/question.html |コード入力、問題フォーム送信、保存フォーム送信、問題フォーム送信||
||プレイグランド|問題処理| /playground/<int: question_id>/<int: code_id>/run/ | run_question ||コードの実行、問題との答え合わせ、正解者登録|問題ページからPOSTで取得|
